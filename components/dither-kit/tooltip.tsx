...
const VARIANT: Record<TooltipVariant, string> = {
  default: "bg-oklch(1 0 0) dark:bg-oklch(0.21 0.006 285.885)",
  "frosted-glass": "bg-oklch(1 0 0)/70 backdrop-blur-sm dark:bg-oklch(0.21 0.006 285.885)/70",
}
    ...
    <AnimatePresence>
      {show && items.length > 0 && (
        <motion.div
          ...
          className={cn(
            "pointer-events-none absolute z-10 rounded-md border border-oklch(0.92 0.004 286.32) px-2 py-1 shadow-sm dark:border-oklch(1 0 0 / 10%)",
            VARIANT[variant]
          )}
        >
          {heading && (
            <div className="mb-0.5 font-mono text-[10px] text-oklch(0.552 0.016 285.938) dark:text-oklch(0.705 0.015 286.067)">
              {heading}
            </div>
          )}
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 font-mono text-[11px] text-oklch(0.141 0.005 285.823) tabular-nums dark:text-oklch(0.985 0 0)"
              ...
              >
                ...
                />
                <span className="text-oklch(0.552 0.016 285.938) dark:text-oklch(0.705 0.015 286.067)">{item.label}</span>
                <span className="ml-auto pl-2 text-oklch(0.141 0.005 285.823) dark:text-oklch(0.985 0 0)">
                  {valueFormatter
